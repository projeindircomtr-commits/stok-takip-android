...
class YakitFragment : Fragment() {
    ...
    private var sonYuklenenListe: List<YakitKaydi> = emptyList()
    ...
    private fun csvDisaAktar() {
        val basliklar = listOf("Plaka", "Yakıt Tipi", "Litre", "Birim Fiyat", "Tutar", "Veren", "Telefon", "Tarih")
        val satirlar = sonYuklenenListe.map {
            val kisi = listOfNotNull(it.verilenAd, it.verilenSoyad).joinToString(" ").trim()
            val tutar = if (it.birimFiyat != null) "%.2f".format(it.birimFiyat * it.litre) else ""
            listOf(
                it.plaka, if (it.yakitTipi == "mazot") "Mazot" else "Benzin",
                "%.2f".format(it.litre), it.birimFiyat?.let { f -> "%.2f".format(f) } ?: "",
                tutar, kisi, it.verilenTelefon ?: "", it.createdAt?.let { t -> tarihiBicimlendir(t) } ?: ""
            )
        }
        com.salman.stoktakip.util.ExportYardimcisi.csvPaylas(requireContext(), "yakit_takip", basliklar, satirlar)
    }

    private fun pdfDisaAktar() {
        val basliklar = listOf("Plaka", "Tip", "Litre", "Tarih")
        val satirlar = sonYuklenenListe.map {
            listOf(it.plaka, if (it.yakitTipi == "mazot") "Mazot" else "Benzin",
                "%.1f".format(it.litre), it.createdAt?.let { t -> tarihiBicimlendir(t) } ?: "")
        }
        com.salman.stoktakip.util.ExportYardimcisi.pdfPaylas(requireContext(), "yakit_takip", "Yakıt Takip Raporu", basliklar, satirlar)
    }
