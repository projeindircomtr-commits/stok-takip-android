...
class DashboardFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.cardBekleyenSenkron.txtBaslik.text = "Güncellenecek Stok"
        ...
        binding.editAra.setOnEditorActionListener { _, _, _ ->
            ...
            if (sorgu.isNotEmpty()) git(MalzemelerFragment.aramaIle(sorgu))
            true
        }
    ...
    private fun aramaAcKapaKur() {
        var acik = false
        binding.btnAramaAcKapa.setOnClickListener {
            acik = !acik
            androidx.transition.TransitionManager.beginDelayedTransition(binding.root as android.view.ViewGroup)
            if (acik) {
                binding.txtBaslik.visibility = View.GONE
                binding.rowAramaGenis.visibility = View.VISIBLE
                binding.btnAramaAcKapa.setImageResource(android.R.drawable.ic_menu_close_clear_cancel)
                binding.editAra.requestFocus()
                val imm = requireContext().getSystemService(android.content.Context.INPUT_METHOD_SERVICE) as android.view.inputmethod.InputMethodManager
                imm.showSoftInput(binding.editAra, android.view.inputmethod.InputMethodManager.SHOW_IMPLICIT)
            } else {
                binding.rowAramaGenis.visibility = View.GONE
                binding.txtBaslik.visibility = View.VISIBLE
                binding.btnAramaAcKapa.setImageResource(android.R.drawable.ic_menu_search)
                binding.editAra.text?.clear()
                val imm = requireContext().getSystemService(android.content.Context.INPUT_METHOD_SERVICE) as android.view.inputmethod.InputMethodManager
                imm.hideSoftInputFromWindow(binding.editAra.windowToken, 0)
            }
        }
    }
    ...
    private fun senkronizeEt(repo: com.salman.stoktakip.data.repo.StokRepository) {
        viewLifecycleOwner.lifecycleScope.launch {
            try {
                ...
                if (isAdded) Snackbar.make(binding.root, com.salman.stoktakip.util.agHatasiniAcikla(e), Snackbar.LENGTH_LONG).show()
            }
        }
    }
