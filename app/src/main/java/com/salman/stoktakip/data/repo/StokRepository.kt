...
import com.salman.stoktakip.util.agHatasiniAcikla
...
class StokRepository(
    ...
    suspend fun tumVeriyiYenile(): Resource<Unit> {
        ...
        return try {
            ...
            Resource.Hata(agHatasiniAcikla(e))
        }
    }
        ...
        return try {
            ...
            for (kayit in bekleyenler) {
                try {
                    ...
                    val teknikHata = "${e.javaClass.simpleName}: ${e.message}"
                    db.bekleyenIslemDao().hataYaz(kayit.id, teknikHata) // sadece ic kayit/teshis icin
                    hataOldu = true
                    sonHataMesaji = agHatasiniAcikla(e)
                }
            }

            tumVeriyiYenile()
            if (hataOldu) Resource.Hata("Bazı kayıtlar gönderilemedi: $sonHataMesaji")
